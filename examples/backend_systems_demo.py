...
from ariadne.core import get_logger, configure_logging
...
def demo_health_checking():
    ...
    healthy_check = create_circuit_based_health_check(
        "healthy", 
        healthy_backend.simulate, 
        simple_circuit
    )
    
    unhealthy_check = create_circuit_based_health_check(
        "unhealthy", 
        unhealthy_backend.simulate, 
        simple_circuit
    )
...
def demo_backend_pooling():
    ...
    pool = create_backend_pool(
        backend_name="demo_pool",
        backend_class=MockPooledBackend,
        min_instances=2,
        max_instances=5
    )
    ...
    for backend in [BackendType.QISKIT, BackendType.STIM, BackendType.CUDA]:
        ...
        if summary:
            ...
            if "execution_time" in summary["metrics"]:
                ...
                print(f"  Execution time: avg {exec_time['mean']:.3f}s, min {exec_time['min']:.3f}s, max {exec_time['max']:.3f}s")
            
            if "memory_usage" in summary["metrics"]:
                memory = summary["metrics"]["memory_usage"]
                print(f"  Memory usage: avg {memory['mean']:.1f}MB, min {memory['min']:.1f}MB, max {memory['max']:.1f}MB")
