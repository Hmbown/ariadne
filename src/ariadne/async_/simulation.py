...
class AsyncSimulator:
    ...
    def __init__(self, max_concurrent_simulations: int = 4, thread_pool_size: Optional[int] = None):
        """
    ...
    async def simulate_batch(
        self, 
        requests: List[AsyncSimulationRequest],
...
class AsyncBackendInterface:
    ...
    async def check_backend_health(self, backend: BackendType) -> Dict[str, Any]:
        ...
        result = await loop.run_in_executor(
            None,
            self._health_checker.check_backend_health,
            backend
        )
    ...
    async def check_all_backends_health(self) -> Dict[str, Dict[str, Any]]:
        ...
        tasks = [
            ...
            for backend in list(BackendType)
        ]
    ...
    async def monitor_backend_performance(
        ...
        while time.time() - start_time < duration:
            # Get current metrics
            loop = asyncio.get_event_loop()
            backend_metrics = await loop.run_in_executor(
                None,
                self._get_backend_metrics,
                backend
            )
...
_global_async_simulator: Optional[AsyncSimulator] = None
