...
from ..core import get_logger, BackendUnavailableError, SimulationError
...
class FallbackResult:
    ...
    @property
    def successful_backend(self) -> Optional[BackendType]:
...
class BackendFallbackManager:
    ...
    def get_fallback_chain(
        ...
        initial_backend: Optional[BackendType] = None
    ) -> List[BackendType]:
        """
        ...
        if circuit:
            ...
            if circuit.num_qubits > 25:
                ...
                print(f"DEBUG: Using large circuit fallback chain ({circuit.num_qubits} qubits): {[b.value for b in chain]}")
                self.logger.debug(f"Using large circuit fallback chain ({circuit.num_qubits} qubits): {[b.value for b in chain]}")
                return chain
...
_global_fallback_manager: Optional[BackendFallbackManager] = None
