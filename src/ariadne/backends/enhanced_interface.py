...
class EnhancedBackendInterface(ABC):
    ...
    def get_optimization_recommendations(self, circuit: QuantumCircuit) -> List[str]:
        ...
        if circuit.num_qubits > capabilities.typical_qubits:
            recommendations.append(f"Circuit has {circuit.num_qubits} qubits, which is larger than typical for this backend ({capabilities.typical_qubits})")
...
class EnhancedBackendWrapper:
    ...
    def __init__(self, backend: Any, backend_name: str, capabilities: Optional[BackendCapabilities] = None):
        """
    ...
    def can_simulate(self, circuit: QuantumCircuit, **kwargs) -> Tuple[bool, str]:
        ...
        if circuit.num_qubits > self._capabilities.max_qubits:
            return False, f"Circuit has {circuit.num_qubits} qubits, backend supports max {self._capabilities.max_qubits}"
    ...
    def get_optimization_recommendations(self, circuit: QuantumCircuit) -> List[str]:
        ...
        if circuit.num_qubits > self._capabilities.typical_qubits:
            recommendations.append(f"Circuit has {circuit.num_qubits} qubits, which is larger than typical for this backend ({self._capabilities.typical_qubits})")
