...
class BackendPerformanceMonitor:
    ...
    def __init__(self, alert_callback: Optional[Callable[[PerformanceAlert], None]] = None):
        """
    ...
    def _check_alerts(self, metric: PerformanceMetric) -> None:
        ...
        if severity:
            ...
            # Log alert
            self.logger.warning(
                f"Performance alert for {metric.backend.value}: {alert.message}"
            )
    ...
    def get_backend_profile(self, backend: BackendType) -> Optional[BackendPerformanceProfile]:
        """
...
_global_performance_monitor: Optional[BackendPerformanceMonitor] = None
