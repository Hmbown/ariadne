...
from ..core import get_logger, BackendUnavailableError
...
class BackendPool:
    ...
    def _create_instance(self) -> Optional[PooledBackend]:
    ...
    def _perform_maintenance(self) -> None:
        ...
        with self._lock:
            ...
            instances_to_remove = min(
                len(idle_instances),
                max(0, len(self._all_instances) - self.min_instances)
            )
            ...
            error_instances = [
                instance for instance in self._all_instances
            ...
            if available_count < self.min_instances and total_count < self.max_instances:
                instances_to_create = min(
                    self.min_instances - available_count,
                    self.max_instances - total_count
                )
    ...
    def _update_stats(self) -> None:
        ...
        if self._stats.total_instances == 0:
            self._status = PoolStatus.ERROR
            self.logger.debug(f"Pool status set to ERROR: total_instances=0")
        elif self._stats.available_instances == 0:
            self._status = PoolStatus.EXHAUSTED
            self.logger.debug(f"Pool status set to EXHAUSTED: available_instances=0")
        elif self._stats.total_instances == self.min_instances:
            self._status = PoolStatus.READY
            print(f"DEBUG: Pool status set to READY: total_instances={self._stats.total_instances} == min_instances={self.min_instances}")
            self.logger.debug(f"Pool status set to READY: total_instances={self._stats.total_instances} == min_instances={self.min_instances}")
        else:
            self._status = PoolStatus.READY
            print(f"DEBUG: Pool status set to READY: total_instances={self._stats.total_instances}, min_instances={self.min_instances}")
            self.logger.debug(f"Pool status set to READY: total_instances={self._stats.total_instances}, min_instances={self.min_instances}")
...
class BackendPoolManager:
    ...
    def get_pool(self, backend_name: str) -> Optional[BackendPool]:
        """
