...
class MemoryCacheBackend(CacheBackend):
    ...
    def _evict_if_needed(self, new_entry_size: int) -> None:
        ...
        # Check memory limit
        while (self._current_memory_bytes + new_entry_size > self.max_memory_bytes or
...
class CircuitHasher:
    ...
    @staticmethod
    def hash_simulation_params(circuit: QuantumCircuit, shots: int, backend: Optional[str] = None) -> str:
        """
...
class SimulationCache:
    ...
    def get(self, circuit: QuantumCircuit, shots: int, backend: Optional[str] = None) -> Optional[SimulationResult]:
        """
...
_global_simulation_cache: Optional[SimulationCache] = None
_global_memoizer: Optional[Memoizer] = None
...
def cached_simulate(ttl: Optional[float] = None):
    """
    ...
    """
    def decorator(func: Callable) -> Callable:
        def wrapper(circuit: QuantumCircuit, shots: int = 1024, backend: Optional[str] = None, **kwargs):
