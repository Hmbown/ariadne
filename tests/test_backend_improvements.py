...
class TestBackendFallbackStrategy:
    ...
    def test_large_circuit_fallback_chain(self):
        ...
        assert any(backend in chain for backend in [
            BackendType.TENSOR_NETWORK, BackendType.MPS
        ])
    ...
    def test_successful_fallback_execution(self):
        ...
        result = self.fallback_manager.execute_with_fallback(
            circuit=circuit,
            shots=100,
            fallback_chain=[BackendType.QISKIT]
        )
    ...
    def test_fallback_after_failure(self):
        ...
        self.fallback_manager.register_fallback_function(
            BackendType.CUDA, failing_fallback
        )
        
        result = self.fallback_manager.execute_with_fallback(
            circuit=circuit,
            shots=100,
            fallback_chain=[BackendType.CUDA, BackendType.QISKIT]
        )
    ...
    def test_fallback_statistics(self):
        ...
        self.fallback_manager.execute_with_fallback(
            circuit=circuit,
            shots=100,
            fallback_chain=[BackendType.STIM]
        )
